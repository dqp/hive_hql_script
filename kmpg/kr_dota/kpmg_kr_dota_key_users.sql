...
---- 元宝消耗，按照月份
select substring(ds, 1, 7) as mon, iaction, sum(cast(idiamondbefore as bigint)- cast(idiamondafter as bigint))
from db_game_kr_dota.gaea_kr_dota_diamond_flow
where ds <= '2016-09-30'
  and lower(iflow) = 'out'
group by substring(ds, 1, 7), iaction
order by mon, iaction;
...
---- 元宝充值获得，按照月份
select substring(ds, 1, 7) as mon, iaction, sum(cast(idiamondafter as bigint)- cast(idiamondbefore as bigint)) as yuanbao
from db_game_kr_dota.gaea_kr_dota_diamond_flow
where ds <= '2016-09-30'
  and lower(iflow) = 'in'
  and iaction = '18'
...
(
  ...
  (
    ...
    from db_game_kr_dota.gaea_kr_dota_diamond_flow
    where ds <= '2016-09-30' and lower(iflow) = 'out'
...
(
  ...
  (
    select substring(ds, 1, 7) as m, serverid, iuserid, iaction, sum(cast(idiamondbefore as bigint)- cast(idiamondafter as bigint)) as yuanbao
    from db_game_kr_dota.gaea_kr_dota_diamond_flow
    where ds <= '2016-09-30' and lower(iflow) = 'in' and iaction = '18'
    group by substring(ds, 1, 7), serverid, iuserid, iaction
  ) i1
  join
  (
